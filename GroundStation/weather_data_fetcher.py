...
def GetAirQuality():
    ...
    try:
        ...
        if not res:
            if log:
                log_queue.put("GetAirQuality Error: 'res' is None")
            return None
        current = res[0].Current()
        if not current:
            if log:
                log_queue.put("GetAirQuality Error: 'current' is None")
            return None
        aqi = round(current.Variables(0).Value())
        if log:
            log_queue.put(f"AQI: {aqi}")
        return aqi
    except Exception as e:
        if log:
            log_queue.put(f"GetAirQuality Error: {e}")
